    ...
    try:
        ...
        for ind, row in enumerate(params.get('table_view').get('rows')):
            ...
            values = {'expense_category_id': row.get('expense_category_id'),
                      'request_nrs': row.get('request_nrs'), 'request_usd': row.get('request_usd'), 'request_sdr': row.get('request_sdr'),
                      'response_nrs': row.get('response_nrs'), 'response_usd': row.get('response_usd'), 'response_sdr': row.get('response_sdr'),
                      'with_held_nrs': row.get('with_held_nrs'), 'with_held_usd': row.get('with_held_usd'), 'with_held_sdr': row.get('with_held_sdr'),
